...
from qiskit.algorithms.optimizers import ADAM, CG, AQGD, COBYLA
...
def linear_operator(param, pad=True, circuit = False):
    ...
    if pad==True:
        ...
        qc.u(param[0], param[1], param[2], data[0])
        qc.u(param[3], param[4], param[5], data[1])
        qc.cx(data[0], data[1])
    elif pad==False:
        data = QuantumRegister(1)
        qc = QuantumCircuit(data)
        qc.u(param[0], param[1], param[2], data)
